...
from users.forms import PreCustomerForm, CustomerForm, CustomerCreateForm
from users.models import Customer
from payments.iugu_helper import create_account_iugu
from posts.tasks import create_free_post


class ThemeChoiceView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if plan:
            ...
            if customer:
                ...
                customer.save()
                request.session['customer'] = customer.pk
                return redirect(reverse_lazy('signup:create_account'))
        return redirect(reverse_lazy('signup:plan_choice'))
...
class CreateAccountView(View):
    ...
    def get(self, request, *args, **kwargs):
        self.context['form'] = CustomerCreateForm()
        return render(request, self.template_name, self.context)

    def post(self, request, *args, **kwargs):
        form = CustomerCreateForm(request.POST, instance=self.customer)
        if form.is_valid():
            customer = form.save(commit=False)
            ...
            if customer.plan != "free" and customer.plan_status is None:
